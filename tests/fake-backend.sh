...
initialize() {
  ...
  : > "$state_file"
  for led in $leds; do
    printf '%s\toff\t0\t0\t0\t0\n' "$led" >> "$state_file"
  done
}

show_status() {
  local led state brightness red green blue
  while IFS=$'\t' read -r led state brightness red green blue; do
...
brightness="" red="" green="" blue="" state=on
while [ "$#" -gt 0 ]; do
  case "$1" in
...
case " $leds " in
  *" $target "*) update "$target" "$state" "$brightness" "$red" "$green" "$blue" ;;
